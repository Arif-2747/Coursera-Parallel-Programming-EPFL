...
case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body]) extends Quad:
  val (mass, massX, massY) =
    ...
    val cx = bodies.map(b => b.x * b.mass).sum / totalMass
    val cy = bodies.map(b => b.y * b.mass).sum / totalMass
    (totalMass, cx, cy)
  ...
  def insert(b: Body): Quad =
    if size > minimumSize then
      ...
      (bodies :+ b).foldLeft(fork: Quad)(_ insert _)
    else
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix =
    val x = math.min(((b.x - boundaries.minX) / sectorSize).toInt, sectorPrecision - 1)
    val y = math.min(((b.y - boundaries.minY) / sectorSize).toInt, sectorPrecision - 1)
    val clampedX = math.max(0, x)
    val clampedY = math.max(0, y)
    this(clampedX, clampedY) += b
    this
