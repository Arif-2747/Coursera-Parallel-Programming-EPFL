...
class ListsSuite extends munit.FunSuite:
  ...
  test("one plus one is three (0pts)?") {
    assert(1 + 1 == 2) // Fixed: changed 3 to 2
  }
  ...
  test("details why one plus one is not three (0pts)") {
    assertEquals(1 + 1, 2) // Fixed: changed 3 to 2
  }
  ...
  test("max of negative numbers") {
    assertEquals(max(List(-5, -2, -9)), -2)
  }

  test("max with repeated elements") {
    assertEquals(max(List(4, 4, 1, 4, 2)), 4)
  }
